...
from common.constants import MINERU_DEFAULT_CONFIG, MINERU_ENV_KEYS, PADDLEOCR_DEFAULT_CONFIG, PADDLEOCR_ENV_KEYS, VIETOCR_DEFAULT_CONFIG, VIETOCR_ENV_KEYS, LLMType
...
class TenantLLMService(CommonService):
    ...
    @classmethod
    def _collect_vietocr_env_config(cls) -> dict | None:
        cfg = VIETOCR_DEFAULT_CONFIG.copy()
        found = False
    ...
    @classmethod
    @DB.connection_context()
    def ensure_vietocr_from_env(cls, tenant_id: str) -> str | None:
        """
        Ensure a VietOCR model exists for the tenant if env variables are present.
        Return the existing or newly created llm_name, or None if env not set.
        """
        cfg = cls._collect_vietocr_env_config()
        if not cfg:
            return None

        saved_vietocr_models = cls.query(tenant_id=tenant_id, llm_factory="VietOCR", model_type=LLMType.OCR.value)
        ...
        for item in saved_vietocr_models:
            api_cfg = _parse_api_key(item.api_key)
            normalized = {k: api_cfg.get(k, VIETOCR_DEFAULT_CONFIG.get(k)) for k in VIETOCR_ENV_KEYS}
            if normalized == cfg:
                return item.llm_name

        used_names = {item.llm_name for item in saved_vietocr_models}
        idx = 1
        base_name = "vietocr-from-env"
        while True:
            candidate = f"{base_name}-{idx}"
            if candidate in used_names:
                idx += 1
